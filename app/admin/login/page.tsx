...
export default function AdminLoginPage({ searchParams }: AdminLoginPageProps) {
	async function loginAction(formData: FormData) {
		...
		try {
			...
			if (!res.ok) {
				redirect(
					`/admin/login?error=1&next=${encodeURIComponent(safeNextPath)}`,
				);
			}
	...
	return (
		<section className="mx-auto max-w-xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
			...
			<h2 className="mt-2 text-2xl font-semibold text-slate-900">
				Masuk ke CMS Desa Jadimulya
			</h2>
			<p className="mt-3 text-sm leading-6 text-slate-600">
				Login diperlukan untuk membuka modul organisasi, potensi, sejarah,
				demografi, berita, dan media desa.
			</p>
