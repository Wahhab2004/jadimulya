    ...
    <div className="space-y-4">
      <section className="flex flex-col gap-5 rounded-[1.6rem] border border-white/70 bg-white/85 p-5 shadow-[0_24px_44px_-30px_rgba(15,23,42,0.35)] backdrop-blur sm:p-6 lg:rounded-[2rem] lg:p-8">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-700">Dashboard</p>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 sm:text-base">
              Pantau performa konten, pembaruan data desa, dan modul layanan dalam satu workspace yang lebih visual,
              cepat, dan nyaman dipakai harian.
            </p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-4">
          {topSummary.map((item) => (
            <article key={item.label} className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_18px_34px_-28px_rgba(15,23,42,0.4)]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-slate-500">{item.label}</p>
                  <p className="mt-3 text-4xl font-semibold leading-none text-slate-900">{item.value}</p>
                </div>
                <span className={`h-11 w-11 rounded-2xl bg-gradient-to-br ${item.accent}`} />
              </div>
      ...
      <section className="grid gap-4 2xl:grid-cols-[1.9fr_1fr]">
        <article className="rounded-[1.6rem] border border-white/70 bg-white/85 p-5 shadow-[0_24px_44px_-30px_rgba(15,23,42,0.35)] backdrop-blur sm:p-6 lg:rounded-[2rem] lg:p-7">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h3 className="text-2xl font-semibold text-slate-900">Aktivitas Konten</h3>
              <p className="mt-1 text-sm text-slate-500">Tren update modul dan publikasi per bulan</p>
            </div>
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" /> Terbit</span>
              <span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-100" /> Draft</span>
            </div>
          </div>
          ...
          <div className="mt-8 grid h-[290px] grid-cols-12 items-end gap-2 sm:gap-3">
            {monthlyActivity.map((value, index) => {
              ...
              return (
                <div key={months[index]} className="flex h-full flex-col items-center justify-end gap-2">
                  <div className="flex h-[220px] items-end gap-1.5">
                    <div className="w-4 rounded-full bg-emerald-500/20" style={{ height: `${draftHeight}px` }} />
                    <div className="w-7 rounded-full bg-emerald-500 shadow-[0_14px_26px_-18px_rgba(16,185,129,0.85)]" style={{ height: `${height}px` }} />
                  </div>
        ...
        <article className="rounded-[1.6rem] border border-white/70 bg-white/85 p-5 shadow-[0_24px_44px_-30px_rgba(15,23,42,0.35)] backdrop-blur sm:p-6 lg:rounded-[2rem] lg:p-7">
          <div className="flex items-center justify-between gap-4">
            <h3 className="text-2xl font-semibold text-slate-900">Kesehatan Modul</h3>
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-500">Juli 2026</span>
          </div>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <article className="rounded-[1.6rem] border border-white/70 bg-white/85 p-5 shadow-[0_24px_44px_-30px_rgba(15,23,42,0.35)] backdrop-blur lg:rounded-[2rem] lg:p-6">
          <h3 className="text-xl font-semibold text-slate-900">Publikasi Cepat</h3>
        ...
        <article className="rounded-[1.6rem] border border-white/70 bg-white/85 p-5 shadow-[0_24px_44px_-30px_rgba(15,23,42,0.35)] backdrop-blur lg:rounded-[2rem] lg:p-6">
          <h3 className="text-xl font-semibold text-slate-900">Progress Pengembangan</h3>
            ...
            {[
              ...
              <div key={label}>
                ...
                <div className="h-2.5 rounded-full bg-slate-200">
                  <div className="h-2.5 rounded-full bg-gradient-to-r from-emerald-500 to-teal-500" style={{ width: `${value}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <article className="rounded-[1.6rem] border border-white/70 bg-white/85 p-5 shadow-[0_24px_44px_-30px_rgba(15,23,42,0.35)] backdrop-blur lg:rounded-[2rem] lg:p-6">
          <h3 className="text-xl font-semibold text-slate-900">Ringkasan Tugas</h3>
