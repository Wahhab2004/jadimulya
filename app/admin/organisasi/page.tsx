...
function normalizeUrl(value: string) {
  ...
  if (resolvedValue.startsWith("/")) {
    try {
      return new URL(
        resolvedValue,
        new URL(buildAdminBeUrl("")).origin,
      ).toString();
    } catch {
      return undefined;
    }
...
async function getBackendErrorMessage(response: Response) {
  ...
  if (!rawBody) {
    return `Permintaan ditolak oleh server (${response.status}).`;
  }

  try {
    const payload = JSON.parse(rawBody) as Partial<BackendResponse<unknown>> & {
      error?: string;
    };
    ...
    try {
      ...
      const message =
        error instanceof Error && error.message
          ? error.message
          : "Gagal menyimpan data organisasi.";
      setNotice(message);
      showAdminToast(message, "error");
    } finally {
