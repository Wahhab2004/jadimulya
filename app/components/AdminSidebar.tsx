...
const iconPaths: Record<AdminNavItem["icon"], string> = {
	...
	potensi:
		"M7 13c0 3 2 5 5 5m5-7c0 4-2 8-5 8V6c2 0 5 2 5 5ZM12 10c-2 0-5 2-5 5",
			...
			{items.map((item) => {
				...
				const content = (
					<>
						...
						<span className="min-w-0 flex-1">
							<span
								className={`block truncate text-sm font-medium ${active ? "text-white" : item.enabled ? "text-slate-700" : "text-slate-400"}`}
							>
								{item.label}
							</span>
							<span
								className={`mt-0.5 block truncate text-xs ${active ? "text-sky-100" : item.enabled ? "text-slate-500" : "text-slate-400"}`}
							>
								{item.description}
							</span>
						</span>
						{item.phase === "Next" ? (
							<span
								className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${active ? "border-sky-300/40 text-sky-100" : "border-sky-100 bg-sky-50 text-sky-700"}`}
							>
								Next
							</span>
						) : null}
				...
				if (!item.enabled) {
					return (
						<div
							key={item.label}
							className="group flex items-center gap-3 rounded-2xl border border-slate-200/80 bg-slate-50 px-3 py-3 opacity-85"
						>
							{content}
						</div>
					);
