...
import { initialPotensiItems, loadStoredPotensiItems, type PotensiItem } from '@/lib/potensi-store';
...
function mapApiPotensiToPublic(items: Awaited<ReturnType<typeof getPotensi>>): PotensiItem[] {
  return items
          ...
          news:
            Array.isArray(apiNews.items) && apiNews.items.length > 0
              ? apiNews.items.map((item) => ({
                  ...
                  description: item.excerpt?.trim() || item.content.slice(0, 160),
