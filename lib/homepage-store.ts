...
export const initialHomepageContent: HomepageContent = {
  ...
  heroSlides: [
    {
      id: "slide-1",
      imageUrl: "/images/karsidi-kades.jpeg",
      caption: "Perangkat Desa Jadimulya",
    },
    {
      id: "slide-2",
      imageUrl: "/images/pertanian.webp",
      caption: "Kantor Desa Jadimulya",
    },
    {
      id: "slide-3",
      imageUrl: "/images/balai_desa.jpg",
      caption: "Potensi Pertanian Desa",
    },
    {
      id: "slide-4",
      imageUrl: "/images/gerbang_jadimulya.webp",
      caption: "Potensi Pariwisata Desa",
    },
  ],
  stats: [
    {
  ...
  return {
    ...
    label:
      typeof item.label === "string" && item.label.trim()
        ? item.label
        : (initialHomepageContent.stats.find((stat) => stat.key === item.key)
            ?.label ?? "Statistik"),
    value:
      typeof item.value === "string" && item.value.trim() ? item.value : "0",
...
function mergeStats(stats: HomepageStat[]): HomepageStat[] {
  ...
  return fallback.map(
    (base) => stats.find((item) => item.key === base.key) ?? base,
  );
}
...
export function loadStoredHomepageContent(
  potensiItems: PotensiItem[] = initialPotensiItems,
) {
  ...
  try {
    ...
    const featuredPotensiIds = featuredPotensiIdsRaw.filter((id) =>
      validPotensiIds.has(id),
    );
    ...
    const news = Array.isArray(parsed.news)
      ? parsed.news
          .map((item) => sanitizeNews(item))
          .filter((item): item is HomepageNewsItem => item !== null)
    ...
    return {
      heroTitle:
        typeof parsed.heroTitle === "string" && parsed.heroTitle.trim()
          ? parsed.heroTitle
          : initialHomepageContent.heroTitle,
      heroSubtitle:
        typeof parsed.heroSubtitle === "string" && parsed.heroSubtitle.trim()
          ? parsed.heroSubtitle
          : initialHomepageContent.heroSubtitle,
      slogan:
        typeof parsed.slogan === "string" && parsed.slogan.trim()
          ? parsed.slogan
          : initialHomepageContent.slogan,
      jelajahiLabel:
        typeof parsed.jelajahiLabel === "string" && parsed.jelajahiLabel.trim()
          ? parsed.jelajahiLabel
          : initialHomepageContent.jelajahiLabel,
      jelajahiHref:
        typeof parsed.jelajahiHref === "string" && parsed.jelajahiHref.trim()
          ? parsed.jelajahiHref
          : initialHomepageContent.jelajahiHref,
      logoKabupatenLabel:
        typeof parsed.logoKabupatenLabel === "string" &&
        parsed.logoKabupatenLabel.trim()
          ? parsed.logoKabupatenLabel
          : initialHomepageContent.logoKabupatenLabel,
      logoKabupatenImageUrl:
        typeof parsed.logoKabupatenImageUrl === "string"
          ? parsed.logoKabupatenImageUrl
          : initialHomepageContent.logoKabupatenImageUrl,
      heroSlides:
        heroSlides.length > 0 ? heroSlides : initialHomepageContent.heroSlides,
      stats,
      featuredPotensiIds:
        featuredPotensiIds.length > 0
          ? featuredPotensiIds
          : initialHomepageContent.featuredPotensiIds,
      newsCategories:
        newsCategories.length > 0
          ? newsCategories
          : initialHomepageContent.newsCategories,
